...
# — Front Limbs Missteps Scatter Tool —
def front_limbs_missteps_tool():
    st.button("◀ back to tools", on_click=lambda: st.session_state.update(tool=None))

    st.header("Front Limbs Missteps Scatterplots")
    ...
    df["Front Limbs Missteps"] = df["Front Right"] + df["Front Left"]
    ...
    for run_name in sel_runs:
        ...
        fig = px.scatter(
            ...
            labels={
                "x_jitter":               "Genotype",
                "Front Limbs Missteps":   "Front Limbs Missteps",
                "Gender":                 "Gender",
                "Walking":                "Walking Score"
            },
            title=f"Front Limbs Missteps - Run: {run_name}"
        )
