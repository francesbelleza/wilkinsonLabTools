...
# — Back Limbs Missteps Scatter Tool —
def back_limbs_missteps_tool():
    st.button("◀ back to tools", on_click=lambda: st.session_state.update(tool=None))

    st.header("Back Limbs Missteps Scatterplots")
    ...
    df["Back Limbs Missteps"] = df["Back Right"] + df["Back Left"]
    ...
    for run_name in sel_runs:
        ...
        fig = px.scatter(
            ...
            labels={
                "x_jitter":              "Genotype",
                "Back Limbs Missteps":   "Back Limbs Missteps",
                "Gender":                "Gender",
                "Walking":               "Walking Score"
            },
            title=f"Back Limbs Missteps - Run: {run_name}"
        )
